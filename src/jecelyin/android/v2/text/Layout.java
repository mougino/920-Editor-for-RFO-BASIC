...
import jecelyin.android.v2.text.style.AlignmentSpan;
import jecelyin.android.v2.text.style.LeadingMarginSpan;
...
import com.jecelyin.util.ArrayUtils;
...
//        draw(c, null, null, 0); //jec-
        draw(c, null, null, 0, null, null, null); //jec+
    }
...
//                     int cursorOffsetVertical) { //jec-
            int cursorOffsetVertical, Highlight mHighlight, Paint mLineNumberPaint, Paint mWhiteSpacePaint) { //jec+
        int dtop, dbottom;
        ...
        int lineCount = getLineCount(); //jec+
        int top = 0;
//        int bottom = getLineTop(getLineCount());
        ...
        int previousLineEnd2 = getLineStart(first >= 3 ? first - 3 : 0);
...
//                    hasTab, spans); //jec-
                    hasTab, spans, mWhiteSpacePaint); //jec+
            }
            if(mLineNumberPaint != null)
                c.translate(-mLineNumberWidth, 0);
        }
    ...
    private void drawText(Canvas canvas,
                                 ...
                                 TextPaint paint,
                                 TextPaint workPaint,
//                                 boolean hasTabs, Object[] parspans) { //jec-
                                 boolean hasTabs, Object[] parspans, Paint mWhiteSpacePaint) { //jec+
        char[] buf;
        if (!hasTabs) {
            if (directions == DIRS_ALL_LEFT_TO_RIGHT) {
                if (DEBUG) {
...
//                        h = dir * nextTab(text, start, end, h * dir, parspans); //jec-
    ...
    {
        float textHeight;
        TextPaint tp = mPaint!=null ? mPaint : mWorkPaint;
        
        // 绘制换行符
        mLineBreakPath.reset();
        float path_width = tp.measureText("L");
        // descent为根据当前字体及其大小的基线到下面的距离(正数),ascent则相反
        float mDescent = tp.descent();
        float mAscent = tp.ascent();
        textHeight = mDescent - mAscent;
        /**
         * lineTo在没有moveTo的情况下,默认坐标是0,0；但是要注意,这个坐标是在 "正" 默认坐标是在正字左下角
         */
        // 移到底部中央
        mLineBreakPath.moveTo(path_width * 0.6F, 0);
        // 竖线
        mLineBreakPath.lineTo(path_width * 0.6F, -textHeight * 0.7F);
        // 左箭头
        mLineBreakPath.moveTo(path_width * 0.6F, 0);
        mLineBreakPath.lineTo(path_width * 0.25F, -textHeight * 0.3F);
        // 右箭头
        mLineBreakPath.moveTo(path_width * 0.6F, 0);
        mLineBreakPath.lineTo(path_width * 0.95F, -textHeight * 0.3F);

        // 绘制制表符
        updateTabPath();
    }
    
    public void updateTabPath()
    {
        TextPaint tp = mPaint!=null ? mPaint : mWorkPaint;
        mTabPath.reset();
        float path_width = tp.measureText("\t");
        float textHeight = tp.descent() - tp.ascent();
        // 绘制 >> 符号
        mTabPath.moveTo(0, -textHeight * 0.5F);
        // 绘制箭头下面那部分
        mTabPath.lineTo(path_width * 0.4F, -textHeight * 0.35F);
        // 绘制箭头上面部分
        mTabPath.lineTo(0, -textHeight * 0.2F);
        // two >
        mTabPath.moveTo(path_width * 0.3F, -textHeight * 0.5F);
        // 绘制箭头下面那部分
        mTabPath.lineTo(path_width * 0.7F, -textHeight * 0.35F);
        // 绘制箭头上面部分
