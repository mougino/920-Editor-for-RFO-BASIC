...
static int match_region(char *text, char *syn, char *matchMem, char *memMatch, int *matchLen)
{
    ...
    while (*pattern != NUL)
    {
        ...
        else if (*pattern == '\\') // escaped character
        {
            *p++ = *pattern++; // backslash itself
            *p++ = *pattern++; // escaped character protected by backslash
        }
        else
        {
            *p++ = *pattern++;
...
static int match_text(char **a, char **b, char *memMatch, int case_match)
{
    ...
    //支持\s符号
    else if (*cmp1 == '\\' && *(cmp1 + 1) == 's') // Whitespace character
    {
        if (*(cmp1 + 2) == '*') // \s* = 0 or more whitespace
        {
            //cmp2测试是否有0个或多个空白字符
            cmp1 += 3;
        ...
        else if (*(cmp1 + 2) == '+') // \s+ = 1 or more whitespace
        {
            //至少需要一个空白字符
            int whitenum = 0;
            while (ISWHITE(*cmp2) || ISBREAK(*cmp2))
    ...
    //支持\w符号
    else if (*cmp1 == '\\' && *(cmp1 + 1) == 'w') // Word character
    {
        if (*(cmp1 + 2) == '+') // \w+ = 1 or more word character
        {
            //至少需要一个字符:[a-zA-Z_]
            int kw = 0;
            while (ISKEYWORD(*cmp2))
    ...
    else if (*cmp1 == '\\' && *(cmp1 + 1) == 'b') // Word-Boundary character
    {
        char_u *word;
        word = (char_u*) (cmp2 - 1);
        //如果当前字符是一个word
    ...
    // Support for [X-Y]: range of characters between X and Y
    else if (*cmp1 == '[' && *(cmp1 + 2) == '-' && *(cmp1 + 4) == ']') // range
    {
        if (*(cmp1 + 5) == '*') // [X-Y]* = 0 or more in the range
        {
            while ((*cmp2 >= *(cmp1 + 1) && *cmp2 <= *(cmp1 + 3)))
            {
                cmp2++;
                retLen++;
            }
            cmp1 += 6;
            // If no character found, leave as is
            if(retLen == 0)
                retLen = -1;
        }
        else if (*(cmp1 + 5) == '+') // [X-Y]+ = 1 or more in the range
        {
            int inside = 0;
            while ((*cmp2 >= *(cmp1 + 1) && *cmp2 <= *(cmp1 + 3)))
            {
        ...
        {
            // Test only current character
            if ((*cmp2 >= *(cmp1 + 1) && *cmp2 <= *(cmp1 + 3)))
            {
                cmp1 += 5;
                retLen++;
                cmp2++;
    ...
    {
        //支持\$
        if(*cmp1 == '\\' && *(cmp1+1) == '$') // escaped dollar
            cmp1++;
        //支持\|
        if(*cmp1 == '\\' && *(cmp1+1) == '|') // escaped pipe
            cmp1++;
